...
with tab_disp:
    ...
    with c2:
        with st.expander(
            "🎯 Target — Căutare rapidă cu Enter",
            expanded=True,
        ):
            ...
            # Câmpul de căutare cu Enter (formular)
            with st.form("target_search_form", clear_on_submit=True):
                ...
                submit_produs = st.form_submit_button(
                    "↵ Adaugă rapid",
                    type="primary",
                    use_container_width=True,
                )

            if submit_produs and cautare_produs.strip():
                rezultate = cauta_produse(cautare_produs)

                if len(rezultate) == 1:
                    # Găsit un singur produs -> Adăugare instant cu Enter
                    produs = rezultate[0]
                    ...
                    st.success(f"✓ {produs['nume']} adăugat (+{produs['valoare']:.2f} lei)")
                    st.rerun()

                elif len(rezultate) > 1:
                    # Mai multe rezultate -> le salvăm pentru selecție rapidă
                    st.session_state.target_rezultate = rezultate
                    st.rerun()
                else:
                    st.warning(f"Nu există produsul «{cautare_produs.strip()}».")

            # Dacă sunt mai multe potriviri, afișăm butoane de selecție
            rezultate_multiple = st.session_state.get("target_rezultate", [])
            if rezultate_multiple:
                st.write("**Alege produsul dorit:**")
                for produs in rezultate_multiple:
                    if st.button(
                        f"👉 {produs['nume']} — {produs['valoare']:.2f} lei",
                        key=f"btn_sel_{produs['id']}",
                        use_container_width=True
                    ):
                        s_now = get_session()
                        nou_target = float(s_now["target"]) + float(produs["valoare"])
                ...
                if st.button("✕ Închide lista", key="close_res"):
                    st.session_state.target_rezultate = []
                    st.rerun()

            # Istoricul produselor adăugate în sesiunea curentă
            if st.session_state.target_produse:
                st.divider()
                st.write("**Produse adăugate în această tură:**")
                for i, produs in enumerate(st.session_state.target_produse):
                    col_p1, col_p2 = st.columns([0.85, 0.15])
                    col_p1.write(f"{i + 1}. {produs['nume']} — {float(produs['valoare']):.2f} lei")
                    if col_p2.button("✕", key=f"rem_t_{i}_{produs['id']}"):
                        s_now = get_session()
                        nou_target = max(0.0, float(s_now["target"]) - float(produs["valoare"]))
            ...
            st.divider()
            if st.button("RESET TARGET", key="reset_target_val", use_container_width=True):
                s_now = get_session()
                save_session(
                    s_now["start_comenzi"],
                    s_now["actual_comenzi"],
